...
//        account.deposit(amount);//입금 처리
    ...
    private static void accountList() {
        ...
        for(int i=0;i<accountArray.length;i++) {
            if(accountArray[i]!=null) {
                System.out.println(accountArray[i].getAccountNumber()+accountArray[i].getOwnerName()+accountArray[i].getBalance());
            }
        }
    }//계좌 리스트 메소드 끝.
    private static void createAccount() {
        ...
        String ownerName = sc.next();//소유주 입력
        System.out.print("초기입금액>");
        int balance = sc.nextInt();//초기입금액 입력
        for(int i=0;i<accountArray.length;i++) {
            if(accountArray[i]==null) {
                accountArray[i]
